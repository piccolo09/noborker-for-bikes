...
from ..models import VehicleManufacturer,VehicleVariant,VehicleModel,VehicleCategory
...
class VehicleModelFilter(filters.FilterSet):
    ...
    category = filters.RelatedFilter(VehicleCategoryFilter, field_name='category', queryset=VehicleCategory.objects.all())
...
class VehicleVariantFilter(filters.FilterSet):
    manager = filters.RelatedFilter(ManufacturerFilter, field_name='manager', queryset=ManufacturerFilter.objects.all())
    class Meta:
